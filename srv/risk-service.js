...
module.exports = cds.service.impl(async function () {
    ...
    this.after("READ", Risks, (data) => {
        ...
        risks.forEach((risk) => {
            if (risk.impact >= 100000) {
                risk.criticality = 1; // Alta criticidad (rojo)
            } else {
                risk.criticality = 2; // Media criticidad (amarillo)
            }
        });
    });

    this.before("CREATE", Risks, async (req) => {
        // Simular que el título ya existe
        if (req.data.title === "Duplicated Risk") {
            throw new Error("UNIQUE_CONSTRAINT_VIOLATION");
        }

        if (!req.data.title) {
            throw new Error("A risk must have a title.");
        }
    });
    ...
    // Conectar al servicio externo
    const BPsrv = await cds.connect.to("API_BUSINESS_PARTNER");

    // Handler para leer BusinessPartners con API key
